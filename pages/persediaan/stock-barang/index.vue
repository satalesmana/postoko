<template>
  <div class="q-ma-lg">
    <CustomeTitle icon="inventory" label="Stok Barang" />
      ...
      <q-item class="bg-grey-4">
        <q-item-section>
          <q-item-label class="row no-wrap items-center">
            ...
            <span class="text-bold">List of Stok Barang</span>
...
const columns = ref([
  ...
  { name: "merek", label: "MEREK", field: "merek", align: "left" },
  {
    ...
    label: "SATUAN",
    field: (row) =>
      row.satuan ? `${row.satuan.code} - ${row.satuan.name}` : "-",
    align: "left",
  },
  {
    name: "harga_beli",
    ...
    field: "harga_beli",
    align: "right",
    format: (val) => val?.toLocaleString("id-ID") ?? "0",
  },
  {
    name: "harga_jual",
    label: "HARGA JUAL",
...
const rows = computed(() => store.getters["stockBarang/getListStockBarang"]);
const selected = ref([]);

const onLoadData = () => {
  store.dispatch("stockBarang/fetchListStockBarang");
};

const onCreateData = () => {
  store.commit("stockBarang/clearStockBarangForm");
  router.push("/persediaan/stock-barang/add");
};
...
const onEditItem = () => {
  ...
  store.commit("stockBarang/setStockBarangForm", {
    ...item,
    kategori: item.kategori?._id ?? item.kategori,
    satuan: item.satuan?._id ?? item.satuan,
  });
  router.push(`/persediaan/stock-barang/edit/${item._id}`);
};
...
const onDelete = () => {
  ...
  store.dispatch("stockBarang/deleteData", ids).then(() => {
    selected.value = [];
  });
};
