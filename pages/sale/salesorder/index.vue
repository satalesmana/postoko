      ...
      <q-item class="bg-grey-4">
        <q-item-section>
          <q-item-label class="row no-wrap items-center">
            ...
            <span class="text-bold">Sales Order List</span>
...
const columns = ref([
  { name: "role", label: "ORDER CODE", field: "role" },
  { name: "name", label: "ORDER NAME", field: "name", sortable: true },
  { name: "email", label: "CUSTOMER NAME", field: "email", sortable: true },
  { name: "role", label: "VALIDITY PERIOD", field: "role" },
]);
const rows = ref([]);
const selected = ref([]);
const onLoadData = () => {};
const onCreateData = () => {
  router.push("/sale/salesorder/form-input");
};
const onEditItem = () => {};
const onDelete = () => {};
</script>
